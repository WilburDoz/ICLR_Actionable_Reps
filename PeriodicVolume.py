...
T = 10000                  # How many gradient steps
D = 80                      # How many neurons
...
if om_init_scheme == 0:
    ...
    lambda_equi_init = 0# 0.05        # Initial equivariance loss weighting
    ...
    om = helper_functions.freq_selector_3D(M)
...
if sep_loss_choice == 0:
    ...
    calc_chi = jit(helper_functions.calc_chi_periodicvolume)
...
for counter in range(K):
    ...
    for step in range(T):
        if step%resample_iters == 0:
            ...
            if sample_choice == 0:
                phi = 2*np.pi*np.random.sample([N_rand,3])
            else:
                phi = np.random.sample([N_rand,3])*spread
            I = init_irreps(om, phi)

            phi_pos = 2*np.pi*np.random.sample([N_pos,3])

            if equi_flag:
                phi_shift = np.vstack([np.zeros([1, 3]), 2 * np.pi * np.random.sample([N_shift, 3])])
                phi_full = np.mod(np.reshape(phi_pos[:, None, :] + phi_shift[None, :, :], [N_pos * (N_shift + 1), 3], order='F'), 2 * np.pi)
